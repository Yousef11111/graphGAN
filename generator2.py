    ...
    def __init__(self, n_node, node_emd_init):
        ...
        with tf.compat.v1.variable_scope('generator'):
            self.embedding_matrix = tf.compat.v1.get_variable(name="embedding",
                                                    ...
                                                    trainable=True)
            self.bias_vector = tf.compat.v1.Variable(tf.zeros([self.n_node]))
        ...
        self.all_score = tf.compat.v1.matmul(self.embedding_matrix, self.embedding_matrix, transpose_b=True) + self.bias_vector
