...
#############################################################################
    ...
    def build_generator(self):
        ...
        with tf.compat.v1.variable_scope("generator"):
            self.generator2 = generator2.Generator(n_node=self.n_node, node_emd_init=self.node_embed_init_g)
        ...
        for epoch in range(config.n_epochs):
            ...
            for g_epoch in range(config.n_epochs_gen):
                ...
                for start in start_list:
                    ...
                    self.sess.run(self.generator2.g_updates,
                                  feed_dict={self.generator2.node_id: np.array(node_1[start:end]),
                                             self.generator2.node_neighbor_id: np.array(node_2[start:end]),
                                             self.generator2.reward: np.array(reward[start:end])})
...
###################################################################################
    ...
    def sample(self, root, tree, sample_num, for_d):
        ...
        all_score = self.sess.run(self.generator2.all_score)
        samples = []
        paths = []
        n = 0
...
###############################################################################################
    ...
    def write_embeddings_to_file(self):
        ...
        modes = [self.generator2, self.discriminator]
        for i in range(2):
